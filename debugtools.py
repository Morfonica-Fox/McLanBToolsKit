...
def dump_all_thread_stacks(output_buffer = sys.stdout.buffer):
    """
    ...
    thread_map: Dict[int, threading.Thread] = {t.ident: t for t in threading.enumerate()}
    # 获取所有线程的帧快照；注意：仅Python层，C阻塞时帧为旧状态
    thread_frames: Dict[int, Any] = sys._current_frames()
    ...
    for tid, frame in thread_frames.items():
        ...
        if frame is None:
            output_buffer.write("    [NO PYTHON FRAME] (thread blocked inside C code)\n".encode('utf-8'))
            continue
