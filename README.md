...
- **MOTD 样式解析**：完整支持 Minecraft 颜色代码（§0~§f）、格式代码（§l/§o/§n/§m）、Bedrock 扩展色（§g~§w）和真彩色（§x）
...
```
2026-07-15 21:00:00   3   12   1   4  192.168.1.100  ▶  224.0.2.60   25565  utf-8 [Block ✘] §6My Minecraft Server
```
...
| rich | * | 可选，启用 Rich 终端渲染 |
| colorama | * | 跨平台 ANSI 颜色支持 |
| elevate | * | 自动管理员提权 |
