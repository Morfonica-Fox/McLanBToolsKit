...
class concurrent_dict:
    ...
    def change_capacity(self, capacity: int):
        if not self._lock_on_change.acquire(blocking=False):
            return
        if self.capacity == capacity: # DCL 检查喵
            self._lock_on_change.release() # 虽然感觉没什么用但是写了更规范喵
            return
        try:
            while self._ops_executing.load() > 0: pass
        ...
        finally: self._lock_on_change.release()
    ...
    @_non_atomised_wrapper
    def items_inaccurate(self, last_bucket_index: int = 0): # 遍历字典某个桶里的所有键值对喵 (注: 不保证一次能遍历所有键值对 有锁占用的会直接跳过喵 为了高性能)
        if last_bucket_index >= (1 << self.capacity):
            last_bucket_index = 0
        if not self._buckets_locks[last_bucket_index].acquire(blocking=False): return last_bucket_index+1, [], False
        try:
    ...
    def rmv_inaccurate(self, key):
        h = hash(key)
        suffix = h & ((1 << self.capacity) - 1)
        if not self._buckets_locks[suffix].acquire(blocking=False): return False
        try:
            if key in self._buckets[suffix]:
                del self._buckets[suffix][key]
                self._entry_count.dec()
    ...
    def to_dict(self):
        ...
        while True:
            for index, (bucket, lock) in enumerate(zip(self._buckets, self._buckets_locks)):
